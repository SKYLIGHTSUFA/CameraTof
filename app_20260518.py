...
def get_camera_id(display_name:str) -> str:
    ...
    def stop(self):
        ...
        if self.process:
            try:
                ...
                self.process.kill()
        print(f"[CAM ({self.camera_id})] 🎬 HLS stopped")
...
class CameraWorker:
    ...
    def __init__(self, device, camera_id: str, display_name: str, use_nvenc: bool, gpu_id: int = 0):
    ...
    def capture(self):
        print(f"[CAM ({self.camera_id})] 📷 START capture")
        try:
            try_set_camera_params(self.device, f"({self.camera_id})")
            ...
            if self._use_cuda_debayer:
                try:
                    ...
                    print(f"[CAM ({self.camera_id})] ⚠️ cv2.cuda.setDevice({self.gpu_id}) failed: {e}")
                    self._use_cuda_debayer = False
            ...
            print(f"[CAM ({self.camera_id})] ❌ start error: {e}")
            self.running = False
            return
        ...
        while self.running:
            try:
                ...
                if not self.running:
                    break
                print(f"[CAM ({self.camera_id})] fetch error: {e}")
                time.sleep(0.1)

    def encode(self):
        print(f"[CAM ({self.camera_id})] 🎞 START encode")
        while self.running:
            try:
                ...
                if now - self.last_empty_log > 5.0:
                    print(f"[CAM ({self.camera_id})] ⏳ нет кадров...")
...
def list_cameras(h: Harvester) -> list:
    ...
    for i, di in enumerate(h.device_info_list):
        ...
        display_name = h.device_info_list[i].display_name
        camera_id    = get_camera_id(display_name)
        if camera_id != '404 Not in List':
            cameras.append({
                "index":        i,
